...
import { useSubstrateAuth } from '~/composables/useSubstrateAuth'
...
const isSubstrateAuthenticated = ref(false)
const authError = ref<string | null>(null)
const signInMessage = ref<string | null>(null)
let authComposable: ReturnType<typeof useSubstrateAuth> | null = null
const { status: mapStatus, error: mapError, lastTxHash, mapNote, mapAccount, checkMapped } = useReviveMapAccount()
const { enabled: onChainEnabled } = useOnChainSettings()
const isSubstrateAddress = computed(() => !!address.value && !/^0x[0-9a-fA-F]{40}$/.test(address.value))
const onChainReady = computed(() => onChainEnabled.value && isConnected.value && isSubstrateAddress.value && mapStatus.value === 'mapped')
...
watch(() => gameState.value.status, async (newStatus, oldStatus) => {
  ...
  if (newStatus === 'finished' && oldStatus !== 'finished') {
    ...
    if (onChainReady.value && isHost.value && onChainGameId.value && wordSalt.value) {
      ...
      if (onChainReady.value && gameState.value.onChainGameId) {
...
function requireOnChainReady(actionLabel: string) {
  if (!onChainEnabled.value) {
    contractError.value = 'On-chain features are disabled while migrating to Substrate.'
    return false
  }
  if (!isConnected.value) {
    contractError.value = `Connect your Substrate wallet to ${actionLabel}.`
    return false
  }
  if (!isSubstrateAddress.value || hasOnlyEvmAccounts.value) {
    contractError.value = 'Select a Substrate (SS58) account to use on-chain features.'
    return false
  }
  if (mapStatus.value !== 'mapped') {
    contractError.value = 'Map your Substrate account (pallet-revive) before on-chain actions.'
    return false
  }
  return true
}
...
async function handleStartGameFlow() {
  const wantsOnChain = onChainEnabled.value
  const readyForOnChain = wantsOnChain ? requireOnChainReady('start the game on-chain') : false
  // If on-chain is enabled and ready, create the on-chain game first
  if (readyForOnChain && !onChainGameId.value) {
  ...
  } else if (wantsOnChain && !readyForOnChain) {
    console.warn('[Game] On-chain enabled but not ready; starting off-chain')
  }
  // Proceed to word selection (off-chain)
  generateWordOptions()
}
...
async function handleCreateGameOnChain() {
  if (!requireOnChainReady('create an on-chain game')) return
...
async function handleMapAccount() {
  ...
  if (mapStatus.value === 'mapped') return
  try {
    await mapAccount()
    await checkMapped(true)
  } catch (err: any) {
    authError.value = err.message || 'Failed to map account'
  }
}

// Join game on-chain (called by players)
async function handleJoinGameOnChain() {
  if (!requireOnChainReady('join the on-chain game')) return
...
async function handleStartGame() {
  ...
  if (onChainReady.value && onChainGameId.value && selectedWordLocal.value && !wordSalt.value) {
...
async function handleCommitWordOnChain(word: string) {
  if (!requireOnChainReady('commit the word on-chain')) return
...
async function handleRevealAndScoreOnChain() {
  if (!requireOnChainReady('reveal and score on-chain')) return
  if (!isHost.value) {
...
watch(() => gameState.value.selectedWord, (newWord) => {
  ...
  if (onChainReady.value && newWord && isHost.value && onChainGameId.value && !wordSalt.value) {
...
// Substrate Sign In
async function handleSubstrateSignIn() {
  if (!activeAccount.value) {
    authError.value = 'Please connect your wallet first'
    return
  }

  if (!authComposable) {
    authError.value = 'Auth not initialized yet, please wait'
    return
  }
  ...
  try {
    if (!signInMessage.value) {
      signInMessage.value = authComposable.prepareSignInMessage(activeAccount.value.address)
    }
    await authComposable.signIn(activeAccount.value)
    isSubstrateAuthenticated.value = true
    console.log('[SubstrateAuth] Successfully authenticated:', activeAccount.value.address)
  } catch (err: any) {
    console.error('[SubstrateAuth] Sign in error:', err)
...
// Check if a peer is Substrate-authenticated
function isPeerAuthenticated(peerId: string): boolean {
  if (!authComposable) {
    console.log('[isPeerAuthenticated] No auth composable')
    return false
  }
  const verifiedUsers = authComposable.getVerifiedUsers()
  ...
  return verifiedUsers.has(peerAddress)
}
...
watch(ready, (isReady) => {
  if (isReady && !authComposable) {
    const yroom = getYRoom()
    if (yroom) {
      authComposable = useSubstrateAuth(yroom, chainId.value)
      console.log('[SubstrateAuth] Initialized')
      ...
      if (isConnected.value && address.value) {
        ...
        signInMessage.value = authComposable.prepareSignInMessage(address.value)
      }
    }
  }
})
...
watch([address, isConnected], ([newAddress, newIsConnected], [prevAddress, prevConnected]) => {
  ...
  if (!newIsConnected && prevConnected && prevAddress) {
    authComposable?.clearSignature(prevAddress)
    isSubstrateAuthenticated.value = false
  ...
  if (newIsConnected && newAddress) {
    checkMapped()
    if (authComposable) {
      signInMessage.value = authComposable.prepareSignInMessage(newAddress)
    }
  }
})
</script>
      ...
      <div class="grid md:grid-cols-2 gap-6">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold truncate flex items-center gap-1">
                  ...
                  <span v-if="isPeerAuthenticated(peer.id)" class="text-green-500" title="Verified">🔐</span>
                ...
                <div class="text-xs text-gray-500 dark:text-gray-400 flex items-center gap-1 flex-wrap">
                  ...
                  <!-- Show address if wallet connected (even without verification) -->
          ...
          <div class="pt-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
            ...
            <!-- Substrate Authentication (Optional) -->
            <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3">
              <div class="flex items-start gap-2 mb-2">
                <span class="text-lg">🔐</span>
                <div class="flex-1">
                  <h3 class="text-sm font-semibold text-blue-900 dark:text-blue-100">Optional: Verify with Substrate</h3>
                  <p class="text-xs text-blue-700 dark:text-blue-300 mt-0.5">Sign a message to verify your identity (optional)</p>
                </div>
              </div>
              ...
              <div class="space-y-2">
                <div class="flex items-center gap-2">
                  ...
                  <UButton size="xs" variant="outline" @click="connectFirstExtension">Load Accounts</UButton>
                </div>

                <div v-if="!isConnected" class="space-y-2">
                  <p class="text-xs text-blue-700 dark:text-blue-300">Detected wallets:</p>
                  <div class="flex flex-wrap gap-2">
                    ...
                    <p v-if="extensions.length === 0" class="text-xs text-blue-700/70 dark:text-blue-300/70">
                ...
                <div v-else class="flex items-center justify-between">
                  <div class="text-xs text-blue-700 dark:text-blue-300">
                    Connected as:
                    <span class="font-mono ml-1">{{ address?.slice(0, 6) }}...{{ address?.slice(-4) }}</span>
                  </div>
                  <UButton size="xs" color="red" variant="soft" @click="disconnect">Disconnect</UButton>
                </div>

                <div v-if="hasOnlyEvmAccounts" class="rounded border border-amber-300 bg-amber-50 p-2 text-xs text-amber-700 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-300">
                  Your wallet is exposing only EVM accounts. Switch to a Substrate (SS58) account to continue.
                </div>

                <div v-if="substrateAccounts.length > 1" class="space-y-1">
                  <p class="text-xs text-blue-700 dark:text-blue-300">Active account:</p>
                  <USelect v-model="selectedAccount" :options="substrateAccountOptions" size="xs" />
                </div>

                <div
                  v-if="signInMessage"
                  class="rounded border border-blue-200 bg-white/70 p-2 text-xs text-blue-800 dark:border-blue-800 dark:bg-blue-950/30 dark:text-blue-200"
                >
                  <div class="text-[11px] font-semibold">Message to sign</div>
                  <pre class="mt-1 whitespace-pre-wrap font-mono text-[10px] leading-relaxed">{{ signInMessage }}</pre>
                </div>

                <UButton
                  v-if="isConnected && !isSubstrateAuthenticated"
                  @click="handleSubstrateSignIn"
                  :loading="isSigningIn"
                  color="primary"
                  size="sm"
                  block
                ...
                <div v-if="isSubstrateAuthenticated" class="flex items-center gap-2 text-xs text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 rounded px-2 py-1.5">
                  <span>✓</span>
                  <span>Verified as {{ address?.slice(0, 6) }}...{{ address?.slice(-4) }}</span>
                </div>

                <p v-if="authError" class="text-xs text-red-600 dark:text-red-400">
                  {{ authError }}
                </p>
                <p v-if="walletError" class="text-xs text-red-600 dark:text-red-400">
                  {{ walletError }}
                </p>
              </div>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
          ...
          <div class="space-y-6">
            <!-- On-chain toggle (Substrate) -->
            <div class="bg-amber-50 dark:bg-amber-900/20 rounded-lg p-3">
              <div class="flex items-start gap-2 mb-2">
                <span class="text-lg">⛓️</span>
                <div class="flex-1">
                  <h3 class="text-sm font-semibold text-amber-900 dark:text-amber-100">Enable on-chain (pallet-revive)</h3>
                  <p class="text-xs text-amber-700 dark:text-amber-300 mt-0.5">Optional. Requires mapping your Substrate account before contract calls.</p>
                </div>
              </div>

              <UCheckbox v-model="onChainEnabled" label="Enable on-chain features" />

              <div v-if="onChainEnabled" class="mt-3 space-y-2">
                <UButton
                ...
                <div v-if="mapStatus === 'mapped'" class="text-xs text-green-600 dark:text-green-400">
                  {{ mapNote || 'Account mapped' }}
                  <span v-if="lastTxHash" class="font-mono">Tx: {{ lastTxHash }}</span>
                </div>

                <p v-if="mapError" class="text-xs text-red-600 dark:text-red-400">
                  {{ mapError }}
                </p>
              </div>
            </div>
            ...
            <div class="pt-4">
              ...
              >
                {{ onChainEnabled ? (isCreatingGame ? 'Creating Game On-Chain...' : (isConnected && !onChainGameId ? '🎮 Start Game (On-Chain)' : '🎮 Start Game')) : '🎮 Start Game' }}
              </UButton>
